...
if sys.platform.startswith('linux'):
    ...
    log.info(f"This code is not validated on {sys.platform}.")
    SYSTEM = 'Linux'
    from cucim.skimage import restoration
    import cupy as cp
...
def _deblur(img: np.ndarray, psf: np.ndarray):
...
def _preprocess_channel(ch: int, M: int, czi, name: str, npy_path = None,
    ...
    if run_deblur:
        ...
        if SYSTEM == 'Linux':
            image = np.stack([_deblur(x, psf) for x in image_list])
        elif SYSTEM == 'Win':
